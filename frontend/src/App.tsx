...
export default function App() {
  ...
  // status: READY / RUNNING / STEP n/N / EXIT 0 / ERROR
  let status = "READY";
  let statusKind = "idle";
  if (busy) { status = "RUNNING"; statusKind = "busy"; }
  else if (error) { status = "ERROR"; statusKind = "err"; }
  else if (events.length && index >= events.length - 1) { status = "EXIT 0"; statusKind = "ok"; }
  else if (events.length) { status = `STEP ${index + 1}/${events.length}`; statusKind = "step"; }
    ...
    <div className={styles.app}>
      ...
      <header style={{
        ...
        <span style={{
          ...
          letterSpacing: "2.5px", color: "var(--fg)",
        }}>
          C<span style={{ color: "var(--cyan)" }}>-</span>FORGE
        </span>
        <div style={{ flex: 1 }} />
        <span data-status={statusKind} style={{
          fontFamily: "var(--mono)", fontSize: 11, letterSpacing: "1px",
          color: statusKind === "busy" ? "var(--amber)"
            : statusKind === "err" ? "var(--red)"
            : statusKind === "ok" ? "var(--cyan)"
            : statusKind === "step" ? "var(--fg)"
            : "var(--fg3)",
        }}>{status}</span>
      ...
      <div className={styles.main}>
        <div className={styles.codePane}>
          <div className={styles.codeBar}>
            ...
            <span className={styles.codeFile}>main.c</span>
          </div>
      ...
      <div className={styles.inspector}>
        <div className={styles.insCol}>
          ...
          <pre className={`${styles.insBody} ${current?.stdout ? styles.stdoutText : styles.emptyDim}`}>
            {current?.stdout || "—"}
          </pre>
        </div>
        <div className={styles.insCol}>
            ...
            {current ? (
              <div className={styles.stackList}>
                {current.stack.map((fn, i) => (
                  <div key={fn + i} className={`${styles.stackFrame}${i === 0 ? " " + styles.stackFrameCurrent : ""}`}>
            ...
            {current ? (
              <>
                <div className={styles.metaRow}>
                  <span className={styles.metaItem}>step <b>{index + 1}/{events.length}</b></span>
                  <span className={styles.metaItem}>line <b>{current.line}</b></span>
                  <span className={styles.metaItem}>fn <b>{current.function}()</b></span>
                  <span className={styles.metaItem} style={{ color: "var(--cyan)" }}>{current.event}</span>
                  {current.swap && (
