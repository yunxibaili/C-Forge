...
CFG_PATH = "D:/yunxibaili/算法/_dbg/trace_cfg.json"
...
def capture(step):
        ...
        for sym in syms:
            ...
            if code == gdb.TYPE_CODE_PTR:
                ...
                try:
                    ...
                    v = None
                variables[name] = {"type": type_name(t), "value": v}
                if a is not None:
                    memory.append(
                        {
                ...
                variables[name] = {"type": type_name(t), "fields": fields}
                if a is not None:
                    memory.append(
                        {
                            "address": "0x%x" % (a & 0xFFFFFFFFFFFFFFFF),
    ...
    # resolve pointer targets by name
    for p in pointers:
    ...
    for p in pointers:
        if not p.get("is_struct") or p["null"]:
            continue
        fields_t, target_t = struct_ptr_field_types_from_name(p)
        if fields_t is None:
            continue
        ...
        for fname, ftype in fields_t.items():
            ft = strip_type(ftype)
            if ft.code == gdb.TYPE_CODE_PTR and strip_type(ft.target()) == target_t:
                next_field = fname
                break
        ...
        cur = p["target_addr"]
        try:
            ptr_val = gdb.parse_and_eval("(void*)0")
            # rebuild a typed pointer value for casting
            raw = gdb.Value(p["target_addr"])  # int
        except Exception:
            raw = None
        # construct typed pointer from int address
        try:
            t = strip_type(gdb.parse_and_eval("(typeof(%s))0" % p["name"]))
            ptype = t
        except Exception:
            ptype = None
        hop = 0
        while cur and cur not in seen_addr and hop < 64 and ptype is not None:
            seen_addr.add(cur)
            try:
...
def struct_ptr_field_types_from_name(p):
    """Reconstruct pointer type info by re-reading the symbol by name in current frame."""
    try:
        val = gdb.parse_and_eval(p["name"])
        return struct_ptr_field_types(val)
    except Exception:
        return None, None
